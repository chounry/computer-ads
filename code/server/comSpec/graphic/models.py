...
class Graphic_info(models.Model):
    ...
    processor = models.ForeignKey(Processor,on_delete=models.CASCADE,help_text='eg:CUDA or Stream')
    ...
    def get_mem_cap(self):
        return self.mem_cap+' MB' if(len(str(self.mem_cap)) < 4 ) else str(myUtil.toGBint(self.mem_cap)) + ' GB'
